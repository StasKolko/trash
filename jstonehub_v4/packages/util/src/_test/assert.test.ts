...
describe("[asserts]", () => {
  ...
  describe("[devFrontendAssert]", () => {
    ...
    it("should throw when DEV is true and condition is true", () => {
      ...
      expect(() => devFrontendAssert(true, COMPONENT, MESSAGE)).toThrow(
        `[${COMPONENT}] ${MESSAGE}`,
      );
    });
    ...
    it("should not throw when DEV is true but condition is false", () => {
      ...
      expect(() => devFrontendAssert(false, COMPONENT, MESSAGE)).not.toThrow();
    });

    it("should not throw when DEV is false regardless of condition", () => {
      vi.stubEnv("DEV", false);

      expect(() => devFrontendAssert(true, COMPONENT, MESSAGE)).not.toThrow();
      expect(() => devFrontendAssert(false, COMPONENT, MESSAGE)).not.toThrow();
    });
  });
});
